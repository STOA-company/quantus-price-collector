...
class BrokerDaemonKafka:
    def __init__(self, market_type: MarketType):
        ...
        if not self.kafka_service.is_connected():
            logger.warning("⚠️ Kafka 연결 실패 - 서비스 시작 시 재시도 예정")
    ...
    async def _process_broker_data(self, broker_name: str, data: dict):
        ...
        try:
            ...
            if message_type == 'realtime_data':
                ...
                # Kafka에 발행
                self.kafka_service.publish_raw_data(broker_name, processed_data)
        ...
        while self.running:
            try:
                for broker_name, broker in self.brokers.items():
                    ...
                    # Kafka에 상태 저장
                    self.kafka_service.publish_broker_status(broker_name, status_data)
